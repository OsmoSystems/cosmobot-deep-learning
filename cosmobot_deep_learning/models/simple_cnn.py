...
def create_model(hyperparameters, input_numerical_data_dimension):
    ...
    # Some simple optimization and error metrics for training
    optimizer = keras.optimizers.Adadelta()
    ...
    image_aware_dense = image_to_do_model.get_layer(name="final_dense")
    temp_and_image_add = keras.layers.concatenate(
        [temperature_input, image_aware_dense.output]
    )
    ...
    temperature_aware_model.compile(
        loss_weights={"temp-aware-DO": 1.0, "DO": 0.2},
